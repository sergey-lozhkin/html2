Elm.EventDaddy = Elm.EventDaddy || {};
Elm.EventDaddy.make = function (_elm) {
   "use strict";
   _elm.EventDaddy = _elm.EventDaddy || {};
   if (_elm.EventDaddy.values)
   return _elm.EventDaddy.values;
   var _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _E = _N.Error.make(_elm),
   _J = _N.JavaScript.make(_elm),
   $moduleName = "EventDaddy";
   var Ark = Ark || {};
   ...
   var content = function (config) {
      return function () {
         var info = Ark.Shows.showInfo(Ark.Shows.showDaddy);
         var u = config.step;
         var u2 = 2 * config.step;
         var d2w = config.thumbWidth;
         var smallQuotePar = function ($) {
            return Graphics.Element.width(d2w)(Text.centered(Ark.Parameters.showSmallQuoteText($)));
         };
         var smallQuoteList = function ($) {
            return Graphics.Element.flow(Graphics.Element.down)(F2(function (x,
            y) {
               return {ctor: "::"
                      ,_0: x
                      ,_1: y};
            })(Ark.Utils.spaceY(u2))(List.intersperse(Ark.Utils.spaceY(u))(List.map(smallQuotePar)($))));
         };
         var smallQuote = A3(Ark.Utils.listDef,
         ...
         var p2h = p1h * 5 / 9 | 0;
         var p2w = p1w * 5 / 9 | 0;
         var photos = Graphics.Element.flow(Graphics.Element.down)(List.intersperse(Ark.Utils.spaceY(u))(A2(List.map,
         function ($) {
         ...
         _J.toList([Text.leftAligned(Ark.Parameters.showAuthorText(info.author))
                   ,Text.leftAligned(Ark.Parameters.showTitleText(info.title))
                   ,Ark.Utils.spaceY(u)
                   ,Graphics.Element.width(d1w)(Text.leftAligned(Ark.Parameters.showQuoteText("«Часы пробили неизвестно к чему относившуюся половину»")))
                   ,Graphics.Element.width(d1w)(Text.rightAligned(Ark.Parameters.showSmallQuoteText("В. Набоков, «Приглашение на казнь»")))
                   ,Ark.Utils.spaceY(u)
                   ,Graphics.Element.width(d1w)(Text.leftAligned(Ark.Parameters.showNormalText("Лейтмотивом как пьесы, так и спектакля являются взаимоотношения отцов и детей. Возможно ли преодолеть пропасть между поколениями? Что&nbsp;есть мужское начало&thinsp;&mdash;&thinsp;только ли разрушение и смерть? Что&nbsp;такое одиночество мужчины? Что&nbsp;ждет всех нас впереди? На&nbsp;эти вопросы пытаются найти ответ автор и его персонажи, режиссёр и&nbsp;актеры.")))
                   ,Ark.Utils.spaceY(u)
                   ,Graphics.Element.width(d1w)(Text.leftAligned(Ark.Parameters.showNormalText("Сценическая версия по пьесе А.Строганова «Сатира».")))
                   ,Ark.Utils.spaceY(u)
                   ...
                   ,Ark.Utils.padL(u2)(Text.leftAligned(Ark.Parameters.showNormalText("Серёженька")))
                   ,Ark.Utils.padL(2 * u2)(Text.leftAligned(Ark.Parameters.showNormalText("Дмитрий КОЛЬЦОВ")))
                   ,Ark.Utils.spaceY(u / 2 | 0)
                   ,Ark.Utils.padL(u2)(Text.leftAligned(Ark.Parameters.showNormalText("Лев Александрович Смышляев")))
                   ,Ark.Utils.padL(2 * u2)(Text.leftAligned(Ark.Parameters.showNormalText("Александр ЛИЗНЕНКОВ")))
                   ,Ark.Utils.spaceY(u)
                   ,Graphics.Element.width(d1w)(Text.leftAligned(Ark.Parameters.showNormalText("В спектакле использована музыка Александра Гольдмана в&nbsp;исполнении автора.")))
                   ,Graphics.Element.empty]));
         var pictureBuy = A2(Graphics.Element.flow,
         Graphics.Element.down,
         _J.toList([A2(Graphics.Element.fittedImage,
                   p1w,
                   p1h)(_L.append("assets/",
                   info.image))
                   ,smallQuote
                   ,Ark.Utils.spaceY(u2)
                   ,Graphics.Element.width(d2w)(Text.centered(Ark.Parameters.showCityText("SAN FRANCISCO")))
                   ,Graphics.Element.width(d2w)(Text.centered(Ark.Parameters.showDateText("6 июля 5pm")))
                   ...
                   d2w,
                   Graphics.Element.heightOf(Ark.Paypal.paypalButton_Daddy_20140706),
                   Graphics.Element.middle)(Ark.Paypal.paypalButton_Daddy_20140706)
                   ,Graphics.Element.empty]));
         ...
         _J.toList([A2(Ark.Topbar.topbar,
                   config,
                   Ark.Site.TopEvents)
                   ,Graphics.Element.color(Ark.Parameters.showBackColor)(Graphics.Element.flow(Graphics.Element.down)(A2(List.intersperse,
                   A2(Graphics.Element.spacer,
                   config.contentWidth,
                   u2),
                   _J.toList([Graphics.Element.empty
                             ,Graphics.Element.flow(Graphics.Element.right)(A2(List.intersperse,
                             Ark.Utils.spaceX(u2),
                             _J.toList([Graphics.Element.empty
                                       ,pictureBuy
                                       ,description
   ...
   skeleton,
   Window.width);
   _elm.EventDaddy.values = {_op: _op
                            ,main: main
                            ,skeleton: skeleton
                            ,sidebar: sidebar
                            ,content: content};
...
Elm.Ark.Shows.make = function (_elm) {
      ...
      return function () {
         ...
         var item = function (se) {
            ...
            _J.toList([Graphics.Element.empty
                      ...
                      se.image)))
                      ,Graphics.Element.flow(Graphics.Element.down)(A2(List.intersperse,
                      Ark.Utils.spaceY(u),
                      _J.toList([Graphics.Element.width(textWidth)(Text.leftAligned(Ark.Parameters.showListAuthorText(se.author)))
   ...
   var showMap = Dict.fromList(_J.toList([{ctor: "_Tuple2"
                                          ...
                                          ,_1: {_: {}
                                               ...
                                               ,photos: _J.toList(["satira-01.jpg"
                                                                  ,"satira-02.jpg"])
                                               ,reading: false
                                               ,smallQuote: _J.toList(["Каждый зритель, благодаря нашим спонсорам, получит от устроителей gift&nbsp;card $25 в популярный ресторан."])
                                         ...
                                         ,{ctor: "_Tuple2"
                                          ...
                                          ,_1: {_: {}
                                               ...
                                               ,events: _J.toList([{_: {}
                                                                   ,button: Maybe.Just(Ark.Paypal.paypalButton_Man_20140719)
                                                                   ,city: "SACRAMENTO"
                                                                   ,date: "19 июля 7pm"
                                               ...
                                               ,photos: _J.toList(["man-woman-01.png"
                                                                  ,"man-woman-02.png"
                                                                  ,"man-woman-03.png"
                                                                  ,"man-woman-05.png"])
                                               ,reading: false
   ...
   var sortedPastList = fromListToShows(_J.toList([showJurden
                                                  ,showLastNight]));
   ...
   var sortedShowList = fromListToShows(_J.toList([showDaddy
                                                  ,showManWoman]));
...
Elm.Ark.Footer.make = function (_elm) {
   ...
   var footer = function (config) {
      return function () {
         ...
         _J.toList([_J.toList([textBox("Activities")
                              ,A2(linkBox,
                              "EventsList.html",
                              "Events")])
                   ,_J.toList([A2(linkBox,
                              ...
                              ,linkBox("SupportUsDonations.html")(Ark.Utils.nbsp("Support Us"))])
...
Elm.Ark.Paypal.make = function (_elm) {
   ...
   var paypalButton_Man_20140719 = buyNowButton("AH7U8S38SYLPC");
   ...
   _elm.Ark.Paypal.values = {_op: _op
                            ...
                            ,paypalButton_Man_20140719: paypalButton_Man_20140719
