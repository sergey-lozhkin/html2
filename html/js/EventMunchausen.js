Elm.EventMunchausen = Elm.EventMunchausen || {};
Elm.EventMunchausen.make = function (_elm) {
   "use strict";
   _elm.EventMunchausen = _elm.EventMunchausen || {};
   if (_elm.EventMunchausen.values)
   return _elm.EventMunchausen.values;
   var _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _E = _N.Error.make(_elm),
   _J = _N.JavaScript.make(_elm),
   $moduleName = "EventMunchausen";
   var Ark = Ark || {};
   ...
   var content = function (config) {
      return function () {
         var info = Ark.Shows.showInfo(Ark.Shows.showMunchausen);
         var u = config.step;
         ...
         _J.toList([A3(Maybe.maybe,
                   Graphics.Element.empty,
                   function (c) {
                      return Graphics.Element.width(d1w)(Text.leftAligned(Ark.Parameters.showTitleText(_L.append("Театр ",
                      _L.append(c,
                      " представляет")))));
                   },
                   info.company)
                   ,A2(Ark.Utils.bool,
                   Graphics.Element.empty,
                   Ark.Utils.spaceY(u))(Maybe.isJust(info.company))
                   ,Graphics.Element.width(d1w)(Text.leftAligned(Ark.Parameters.showNormalText("Наверное, это очень символично, по&nbsp;крайней мере, для меня лично, что театр «На&nbsp;БЭКЪЯРДЕ», спустя 12 лет, вновь играет Григория Горина и&nbsp;вновь на&nbsp;сцене Congregation&nbsp;Beth&nbsp;Am. Возможно, кто-то помнит спектакль «Забыть&nbsp;Герострата», несмотря на&nbsp;строгий наказ забыть. Наша организация Creative&nbsp;Association&nbsp;ARK помогает театральным коллективам Bay&nbsp;Area/San&nbsp;Francisco в&nbsp;создании новых спектаклей.")))
                   ,Ark.Utils.spaceY(u)
                   ,Graphics.Element.width(d1w)(Text.leftAligned(Ark.Parameters.showNormalText("Мы рады пригласить Вас на&nbsp;премьеру театра «На&nbsp;БЭКЪЯРДЕ»!")))
                   ,Ark.Utils.spaceY(u)
                   ,Text.leftAligned(Ark.Parameters.showQuoteText("В спектакле принимают участие"))
                   ,Ark.Utils.padL(u2)(Text.leftAligned(Ark.Parameters.showNormalText("Натан БАХ")))
                   ,Ark.Utils.padL(u2)(Text.leftAligned(Ark.Parameters.showNormalText("Игорь АРОНИН")))
                   ,Ark.Utils.padL(u2)(Text.leftAligned(Ark.Parameters.showNormalText("Леонид ФЕДОТОВ")))
                   ,Ark.Utils.padL(u2)(Text.leftAligned(Ark.Parameters.showNormalText("Борис ГРИБАНОВСКИЙ")))
                   ,Ark.Utils.padL(u2)(Text.leftAligned(Ark.Parameters.showNormalText("Ольга НИКИТИНА")))
                   ,Ark.Utils.padL(u2)(Text.leftAligned(Ark.Parameters.showNormalText("Вадим ДАГМАН")))
                   ,Ark.Utils.padL(u2)(Text.leftAligned(Ark.Parameters.showNormalText("Элла МЕЙДБРАЙ")))
                   ,Graphics.Element.empty]));
         ...
         _J.toList([A2(Graphics.Element.fittedImage,
                   ...
                   ,smallQuote
                   ,Ark.Utils.spaceY(u2)
                   ,Graphics.Element.width(d2w)(Text.centered(Ark.Parameters.showCityText("LOS ALTOS HILLS")))
                   ,Graphics.Element.width(d2w)(Text.centered(Ark.Parameters.showDateText("28 сентября 6pm")))
                   ,Graphics.Element.width(d2w)(Text.centered(Ark.Utils.ulink("http://www.betham.org/about/contact")(Ark.Parameters.showPlaceText("Congregation&nbsp;Beth&nbsp;Am"))))
                   ,Ark.Utils.spaceY(u)
                   ,A3(Graphics.Element.container,
                   d2w,
                   Graphics.Element.heightOf(Ark.Paypal.paypalButton_Munchausen_20140928),
                   Graphics.Element.middle)(Ark.Paypal.paypalButton_Munchausen_20140928)
                   ,Graphics.Element.empty]));
   ...
   skeleton,
   Window.width);
   _elm.EventMunchausen.values = {_op: _op
                                 ,main: main
                                 ,skeleton: skeleton
                                 ,sidebar: sidebar
                                 ,content: content};
...
Elm.Ark.Shows.make = function (_elm) {
                                                           ...
                                                           ,_J.toList([_U.replace([["city"
                                                                      ...
                                                                      ,_U.replace([["city"
                                                                                  ...
                                                                                  ,["placeUrl"
                                                                                   ,"http://www.sheltontheater.org/?page_id=102"]
                                                                                  ,["button"
                                                                                   ,Maybe.Just(Ark.Paypal.paypalButton_Decorator_20141102)]],
                                                                      blankEvent)])]],
                                          blankShow)}
                                         ,{ctor: "_Tuple2"
                                          ,_0: showMunchausen
   ...
   var sortedShowList = fromListToShows(_J.toList([showMunchausen
                                                  ,showDecorator]));
...
Elm.Ark.Paypal.make = function (_elm) {
   ...
   var paypalButton_Decorator_20141102 = buyNowButton("8MSH3B88XMCV6");
   ...
   _elm.Ark.Paypal.values = {_op: _op
                            ...
                            ,paypalButton_Decorator_20141102: paypalButton_Decorator_20141102
